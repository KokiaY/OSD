...
class UnetFormerLoss(nn.Module):
    ...
    def __init__(self, ignore_index=255, class_weights=None):
        super().__init__()
        self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index, class_weights=class_weights),
                                   DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
...
class DeltaLoss(nn.Module):
    ...
    def __init__(self, ignore_index=255):
        super().__init__()
        self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                                   WeightedBoundaryDiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
