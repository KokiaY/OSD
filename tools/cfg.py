...
def py2dict(file_path: Union[str, Path]) -> dict:
    ...
    unique_suffix = hashlib.md5(str(file_path).encode("utf-8")).hexdigest()
    unique_module_name = f"_cfg_{module_name}_{unique_suffix}"

    sys.path.insert(0, config_dir)
    try:
        spec = importlib.util.spec_from_file_location(unique_module_name, file_path)
        if spec is None or spec.loader is None:
            raise ImportError(f"Cannot load config module from {file_path}")
