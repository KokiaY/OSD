...
# Default to the SwinV2 experiment configs. Override from the shell if needed.
# Example:
# GPU_ID=0 SOS_PALSAR_CONFIG=config/sos/d2ls_swin_weighted_palsar.py bash run_parallel_sos_m4d_mados.sh
MADOS_CONFIG="${MADOS_CONFIG:-config/mados/d2ls_swin_weighted.py}"
SOS_PALSAR_CONFIG="${SOS_PALSAR_CONFIG:-config/sos/d2ls_swin_weighted_palsar.py}"
SOS_SENTINEL_CONFIG="${SOS_SENTINEL_CONFIG:-config/sos/d2ls_swin_weighted_sentinel.py}"

# Single-GPU sequential mode.
GPU_ID="${GPU_ID:-0}"

LAUNCH_LOG="train_logs/sequential_${RUN_ID}.jobs"
MASTER_LOG="train_logs/sequential_${RUN_ID}.log"
touch "$LAUNCH_LOG"
