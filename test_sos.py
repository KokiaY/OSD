...
def get_args():
    ...
    arg("-b", "--batch_size", type=int, default=None, help="Batch size for testing")
    ...
    arg("--max_visualizations", type=int, default=None, help="Maximum number of panel images to save")
...
def get_palette(config):
    ...
    if palette is None:
        raise ValueError("Config must provide PALETTE for SOS visualization.")
    return np.asarray(palette, dtype=np.uint8)


def build_sensor_lookup(dataset):
    ...
    for sample in samples:
        ...
        sensor_name = sample.get("sensor")
        if image_id is None or sensor_name is None:
            continue
        sensor_lookup[str(image_id)] = str(sensor_name)

    if not sensor_lookup:
        return {}, None
    return sensor_lookup, "sensor"
...
def build_metrics_report(evaluator, class_names):
    ...
    summary = {
        ...
        "mean_dice": safe_float(np.nanmean(dice)),
...
def build_sensor_reports(sensor_evaluators, sensor_image_counts, class_names):
    sensor_reports = {}
    for sensor_name in sorted(sensor_evaluators):
        summary, per_class, confusion = build_metrics_report(sensor_evaluators[sensor_name], class_names)
        summary["num_images"] = int(sensor_image_counts.get(sensor_name, 0))
        sensor_reports[sensor_name] = {
            "summary": summary,
            "per_class": per_class,
            "confusion": confusion,
        }
...
def print_sensor_metrics(sensor_reports, sensor_label):
    if not sensor_reports:
        return
    print("Per-{} metrics:".format(sensor_label))
    for sensor_name, report in sensor_reports.items():
        summary = report["summary"]
        per_class = report["per_class"]
        print("[{}]".format(sensor_name))
        print(
            "  images: {num_images:>4} | OA: {oa} | mIoU: {miou} | mF1: {mf1} | mPrecision: {mp} | mRecall: {mr}".format(
...
def cleanup_overall_metrics(output_path):
    metrics_dir = output_path / "metrics"
    for filename in [
        "summary.json",
        "summary.txt",
...
def save_sensor_metrics(output_path, sensor_reports, sensor_label):
    if not sensor_reports:
        return
            ...
            sensor_class_rows.append(
                {
                    ...
                    "tp_pixels": item["tp_pixels"],
                    "fp_pixels": item["fp_pixels"],
                    "fn_pixels": item["fn_pixels"],
    ...
    with open(metrics_dir / f"per_{sensor_label}_summary.json", "w", encoding="utf-8") as json_file:
        json.dump(sensor_rows, json_file, indent=2, ensure_ascii=False)

    with open(metrics_dir / f"per_{sensor_label}_summary.csv", "w", encoding="utf-8", newline="") as csv_file:
        fieldnames = [
            "sensor_type",
            "sensor_name",
            "num_images",
    ...
    with open(metrics_dir / f"per_{sensor_label}_summary.txt", "w", encoding="utf-8") as txt_file:
        for row in sensor_rows:
            txt_file.write(
                "{sensor_name}: images={num_images}, OA={oa}, mIoU={miou}, mPrecision={mp}, mRecall={mr}, mF1={mf1}\n".format(
                    sensor_name=row["sensor_name"],
                    num_images=row["num_images"],
                    oa=format_metric(row["overall_accuracy"]),
                    miou=format_metric(row["mean_iou"]),
                    mp=format_metric(row["mean_precision"]),
                    mr=format_metric(row["mean_recall"]),
                    mf1=format_metric(row["mean_f1"]),
                )
            )
            for class_row in sensor_class_rows:
                if class_row["sensor_name"] != row["sensor_name"]:
                    continue
                txt_file.write(
                    "  class={class_name}, IoU={iou}, Precision={precision}, Recall={recall}, F1={f1}\n".format(
                        class_name=class_row["class_name"],
                        iou=format_metric(class_row["iou"]),
                        precision=format_metric(class_row["precision"]),
                        recall=format_metric(class_row["recall"]),
                        f1=format_metric(class_row["f1"]),
                    )
                )

    with open(metrics_dir / f"per_{sensor_label}_class_metrics.json", "w", encoding="utf-8") as json_file:
        json.dump(sensor_class_rows, json_file, indent=2, ensure_ascii=False)

    with open(metrics_dir / f"per_{sensor_label}_class_metrics.csv", "w", encoding="utf-8", newline="") as csv_file:
        fieldnames = [
            "sensor_type",
            "sensor_name",
            "class_id",
...
def main():
    ...
    sensor_lookup, sensor_label = build_sensor_lookup(dataset)

    sensor_evaluators = {sensor_name: Evaluator(num_class=config.num_classes) for sensor_name in sorted(set(sensor_lookup.values()))}
    for sensor_evaluator in sensor_evaluators.values():
        sensor_evaluator.reset()
    sensor_image_counts = {sensor_name: 0 for sensor_name in sensor_evaluators}
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(test_loader):
            ...
            for index in range(predictions.shape[0]):
                ...
                sensor_name = sensor_lookup.get(image_id)
                if gt_mask is not None:
                    if sensor_name is not None:
                        sensor_evaluators[sensor_name].add_batch(gt_mask, pred_mask)
                if sensor_name is not None:
                    sensor_image_counts[sensor_name] += 1
    ...
    print("Processed {} images in {:.2f} s ({:.4f} s / image)".format(total_images, inference_time, inference_time / max(total_images, 1)))

    sensor_reports = build_sensor_reports(sensor_evaluators, sensor_image_counts, class_names)
    if not sensor_reports:
        raise ValueError("SOS test dataset does not provide sensor groups, so per-sensor metrics cannot be computed.")

    for report in sensor_reports.values():
        report["summary"]["split"] = args.split
        report["summary"]["tta"] = args.tta
        report["summary"]["checkpoint_path"] = checkpoint_path
        report["summary"]["seconds_per_image"] = safe_float(inference_time / max(total_images, 1))
        report["summary"]["saved_panels"] = saved_panels

    cleanup_overall_metrics(args.output_path)
    print_sensor_metrics(sensor_reports, sensor_label)
    save_sensor_metrics(args.output_path, sensor_reports, sensor_label)
